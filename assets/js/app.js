    ...
    function renderTable() {
        ...
        reversedData.forEach((row, index) => {
            ...
            if (selectedSensor === "all") {
                ...
                let outVal = row[`${selectedSensor}_outdoor`] || 0; let inVal = row[`${selectedSensor}_indoor`] || 0;
                valISPU = inVal; 
                let unit = selectedSensor.includes("pm") ? "µg/m³" : "PPM";
                let namaSensor = selectedSensor.toUpperCase().replace("PM25", "PM 2.5").replace("PM10", "PM 10");
                strOutdoor = `${namaSensor}: ${outVal.toFixed(1)} ${unit}`; strIndoor = `${namaSensor}: ${inVal.toFixed(1)} ${unit}`;
            }

            const status = getIspu(valISPU);
            tr.innerHTML = `<td>${index + 1}</td><td style="color: var(--text-muted); font-family: monospace;">${waktuStr}</td><td style="font-size: 11px;">${strOutdoor}</td><td style="font-size: 11px; font-weight: bold; color: var(--accent-yellow);">${strIndoor}</td><td><span style="background:${status.color}; color:#fff; padding:4px 10px; border-radius:12px; font-size:11px; font-weight:bold;">${status.lbl}</span></td>`;
